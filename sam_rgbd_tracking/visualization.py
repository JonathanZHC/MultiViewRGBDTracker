...
def make_overlay(result: FrameResult) -> np.ndarray:
    ...
    for instance in result.instances:
        ...
        mask = instance.mask
        if mask.any():
            blended = image[mask].astype(np.float32) * 0.55 + color.astype(np.float32) * 0.45
            ...
            text = (
                ...
                f"det={instance.semantic_confidence:.2f} trk={instance.tracking_confidence:.2f}"
            )
            cv2.putText(image, text, (x0, max(18, y0 - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.48, color.tolist(), 1, cv2.LINE_AA)
    return image
...
def make_mask_debug(result: FrameResult, *, raw: bool) -> np.ndarray:
    ...
    for instance in result.instances:
        canvas[instance.raw_mask if raw else instance.mask] = _color(instance.track_id)
    return canvas
...
class RvizPublisher:
    ...
    def __init__(self, node: Any, camera_name: str, config) -> None:
        ...
        self.points_pub = node.create_publisher(PointCloud2, f"{prefix}/visible_points", 2)
        self.markers_pub = node.create_publisher(MarkerArray, f"{prefix}/markers", 2)
        self.overlay_pub = node.create_publisher(Image, f"{prefix}/rgb_overlay", 2)
        self.raw_pub = node.create_publisher(Image, f"{prefix}/raw_masks", 2)
        self.filtered_pub = node.create_publisher(Image, f"{prefix}/filtered_masks", 2)
    ...
    def _pointcloud(self, result: FrameResult, stamp: Any):
        ...
        cloud.header.frame_id = str(self.config.ros.world_frame) if use_world else f"{self.camera_name}_optical_frame"
        cloud.height = 1
        cloud.fields = [
    ...
    def _markers(self, result: FrameResult, stamp: Any):
        ...
        array = MarkerArray()
        frame_id = str(self.config.ros.world_frame) if result.frame.world_from_camera is not None else f"{self.camera_name}_optical_frame"
        for instance in result.instances:
            centroid = instance.centroid_world if result.frame.world_from_camera is not None else instance.centroid_camera
            if centroid is None:
                continue
            ...
            text.pose.position.x, text.pose.position.y, text.pose.position.z = map(float, centroid)
            text.pose.position.z += 0.12
            text.pose.orientation.w = 1.0
            text.scale.z = 0.08
            text.color.r, text.color.g, text.color.b, text.color.a = float(color[0]), float(color[1]), float(color[2]), 1.0
            text.text = f"#{instance.track_id} {instance.label} [{instance.status.value}]"
            ...
            if instance.bbox_min is not None and instance.bbox_max is not None:
                ...
                size = np.maximum(instance.bbox_max - instance.bbox_min, 1e-3)
                ...
                box.color.r, box.color.g, box.color.b, box.color.a = float(color[0]), float(color[1]), float(color[2]), 0.14
                array.markers.append(box)
        return array

    def publish(self, result: FrameResult, stamp: Any) -> None:
        self.points_pub.publish(self._pointcloud(result, stamp))
        self.markers_pub.publish(self._markers(result, stamp))
        overlay = self.bridge.cv2_to_imgmsg(make_overlay(result), encoding="rgb8")
        ...
        if bool(self.config.runtime.publish_debug_images):
            raw = self.bridge.cv2_to_imgmsg(make_mask_debug(result, raw=True), encoding="rgb8")
            filtered = self.bridge.cv2_to_imgmsg(make_mask_debug(result, raw=False), encoding="rgb8")
