...
def main() -> None:
    ...
    rgb_np[..., 3] = 255
    rgb_in = wp.array(rgb_np, dtype=wp.uint8, device=device)
    rgb_out = wp.empty(rgb_np.shape, dtype=wp.uint8, device=device)
    ...
    depth_np[0, 0] = np.inf
    depth_in = wp.array(depth_np, dtype=wp.float32, device=device)
    depth_out = wp.empty(
        depth_np.shape,
        dtype=wp.float32,
        device=device,
    ...
    assert np.array_equal(rgb_result[..., 3], rgb_np[..., 3])
    assert np.any(rgb_result[..., :3] != rgb_np[..., :3])

    assert depth_result.shape == depth_np.shape
    assert depth_result.dtype == np.float32
    assert np.isnan(depth_result[0, 0])
    assert np.isfinite(depth_result).any()

    print("Warp camera corruption sanity check passed")
    print("device:", device)
    print("RGB changed pixels:", int(np.count_nonzero(rgb_result != rgb_np)))
    print("Depth valid pixels:", int(np.count_nonzero(np.isfinite(depth_result))))
    print("Depth invalid pixels:", int(np.count_nonzero(~np.isfinite(depth_result))))
